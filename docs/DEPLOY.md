...
4. `/opt/envo/run envoctl mail-login` — код на экране, вход в браузере **строго под
   support@envo.live** (`ENVO_MAIL_FROM`). Другой аккаунт код отвергает и токен не сохраняет:
   14.09 вход под личным аккаунтом привёл к пяти дублям с личного адреса — письма уходили
   через `/me`, и проверка «уже писали?» смотрела не в тот ящик. Теперь ящик прибит явно.
   Токен в `/var/lib/envo/graph_token.json`.
...
8. **Отправка писем выключена по умолчанию.** Свежая база молчит, пока стоп-кран не снят
   в приложении (Настройки → Почта) или `kv mail.hold_all = '0'`. Вэлкомы уходят только по
   заказам, созданным после первого прогона ядра — историю обработала прежняя автоматика.
9. Параллельная работа с Cowork-рутинами: сутки для лестницы, трое суток для продаж,
